 ...
 */
public class SalaryDAO extends HibernateDaoSupport {
	...
	public Salary findById(java.lang.Integer id) {
		...
		try {
			Salary instance = (Salary) getHibernateTemplate().get(
					"com.po.Salary", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByEmoney(Object emoney) {
		return findByProperty(EMONEY, emoney);
	}
	...
	public Salary merge(Salary detachedInstance) {
		...
		try {
			Salary result = (Salary) getHibernateTemplate().merge(
					detachedInstance);
